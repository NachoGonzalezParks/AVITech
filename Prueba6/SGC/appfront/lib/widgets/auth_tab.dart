  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      try {
        ...
        String errorMessage;
        if (e.code == 'API_ERROR_400' && e.message == 'La cuenta no ha sido verificada.') {
          errorMessage = 'La cuenta no ha sido verificada. Revise su casilla de correos (bandeja de entrada o carpeta de spam).';
        } else if (e.code == 'API_ERROR_401') {
          errorMessage = 'La cuenta o la contraseña ingresada es incorrecta.';
        } else {
          errorMessage = 'Error: ${e.message ?? "Falló la conexión con el servidor"}';
        }

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(errorMessage),
            backgroundColor: Colors.black, // SnackBar negro
...
class __RegisterFormState extends State<_RegisterForm> {
  ...
  Future<void> _checkEmail() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error al verificar email: $e'),
          duration: const Duration(seconds: 30),
          action: SnackBarAction(
            label: 'Aceptar',
            textColor: Colors.white,
  ...
  void _scrollToField(BuildContext context, GlobalKey key) {
    final box = key.currentContext?.findRenderObject() as RenderBox?;
    if (box != null) {
      final position = box.localToGlobal(Offset.zero);
      final scrollableState = _scrollKey.currentContext?.findAncestorStateOfType<ScrollableState>();
  ...
  Future<void> _register() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_selectedCountry == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Seleccione un país')),
        );
        hasError = true;
      }
      ...
      if (_selectedIdType == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Seleccione un tipo de identificación')),
        );
        hasError = true;
      }
      ...
      if (_selectedGender == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Seleccione un sexo')),
        );
        hasError = true;
      }
      ...
      try {
        ...
        try {
          final parsedDate = DateFormat('yyyy-MM-dd').parse(formattedDate);
          formattedDate = DateFormat('yyyy-MM-dd').format(parsedDate);
        } catch (e) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Formato de fecha inválido (use dd-mm-aaaa)')),
          );
          return;
        }
      }
      ...
      // Encontrar el primer campo con error y desplazar a él
      final firstErrorField = _findFirstErrorField();
      if (firstErrorField != null) {
        _scrollToField(context, firstErrorField);
      }
    }
  }

  GlobalKey? _findFirstErrorField() {
    // Lista de todos los campos con sus claves
    final fields = [
      if (_emailController.text.isEmpty) GlobalKey(),
      if (_passwordController.text.isEmpty) GlobalKey(),
      if (_confirmPasswordController.text.isEmpty) GlobalKey(),
      if (_firstNameController.text.isEmpty) GlobalKey(),
      if (_lastNameController.text.isEmpty) GlobalKey(),
      if (_idNumberController.text.isEmpty) GlobalKey(),
      if (_birthDateController.text.isEmpty) GlobalKey(),
    ];
        ...
        children: [
          Expanded(
            child: SingleChildScrollView(
              key: _scrollKey, // Clave para el scroll
              ...
              child: Column(
                children: [
                  ...
                  TextFormField(
                    ...
                    readOnly: false, // Permitir escritura manual
                  ...
                  TextFormField(
                    ...
                    validator: (value) {
                      if (value != null && value.isNotEmpty) {
                        if (!RegExp(r'^[0-9+() -]{6,20}$').hasMatch(value)) {
