...
def registro_usuario(request):   
    ...
    try:
        ...
        if password1 != password2:
            return Response({'success': False, 'message': 'Las contraseñas no coinciden.'}, 
        ...
        try:
            ...
            return Response({'success': False, 
                            'message': f'Contraseña débil: {", ".join(password_error.messages)}'}, 
                           status=status.HTTP_400_BAD_REQUEST)

        # Validar campos obligatorios
        required_fields = ['email', 'nombre', 'apellido', 'pais', 
        ...
        try:
            pais_obj = Paises.objects.get(Nombre=pais)
            tipo_identificacion_obj = TiposIdentificacion.objects.get(
                Codigo=tipo_identificacion, 
                PaisID=pais_obj
            )
            sexo_obj = Sexos.objects.get(Nombre=sexo)
        except (Paises.DoesNotExist, TiposIdentificacion.DoesNotExist, Sexos.DoesNotExist) as e:
        ...
        Personas.objects.create(
            ...
            NroIdentificacion=numero_identificacion,
            SexoID=sexo_obj,
        ...
        # Enviar email de activación (comentado temporalmente para pruebas)
